...
parser.add_argument('-cat', type=int, required=True, help='category column')
parser.add_argument('-scale', nargs="+", type=float, default=[1.0], help='scale factors for multiplication of the count')
parser.add_argument('-ends', type=str, required=False, default='keep', help='count column')
...
for line in filein:
    if line.strip() != '':
        ...
        category = ll[args.cat - 1].strip()
        d[category] = d.get(category, {})
        d[category][position] = d[category].get(position, 0) + 1
...
for category in sorted(d.keys()):
    ...
    for position in sorted(d[category].keys()):
        ...
        if writeFlag:
            out.write(str(position) + '\t' + str(count * float(scaleFactor)) + '\t' + category + '\n')
