...
#files = sorted([fn for fn in glob("dataDir/*cutadapt13.2900K.bowtie_ecoli.sorted.bed.TT.noHS.*.gene.Cov.bed") if not os.path.basename(fn).startswith('STLWT4')]) + sorted(glob("../rnaSeq/dataDir/RNA_*genesCov.nGene.nRead.bed"))
files = sorted([fn for fn in glob("dataDir/STL*cutadapt13.2900K.bowtieNotall.sorted.bed.*.gene.Cov.bed") if not os.path.basename(fn).startswith('STLWT4')]) + sorted(glob("../rnaSeq/dataDir/RNA.*.sorted.genesCov.bed"))
#output = 'dataDir/genesMergedCounts_rep2.csv'
output = 'dataDir/genesMergedCounts_rep2_notAll_rna2.csv'
